...
impl Database {
    pub fn new(options: DatabaseOptions) -> Self {
        ...
        let subscription_db_filename = format!("{}subscription{}", &db_file_prefix, DB_FILE_POSTFIX);
        let banana_state_db_filename = format!("{}banana_state{}", &db_file_prefix, DB_FILE_POSTFIX);
        Self {
    ...
    async fn access_db<DbType>(&self, db_path: &Path, operation: impl FnOnce(&mut DbType) -> DbOperationResult) -> Result<DbType, io::Error>
    where
        DbType: Serialize + DeserializeOwned + Default
    {
        ...
        if file_size == 0 {
            ...
            DbType::default().serialize(&mut serializer).map_err(|e| io::Error::other(e))?;
            db_file.file.seek(std::io::SeekFrom::Start(0)).map_err(|e| io::Error::other(e))?;
        }

        // parse file
        let mut db_contents: DbType = rmp_serde::from_read(&mut db_file.file).map_err(|e| io::Error::other(e))?;
        ...
        if write_needed {
            ...
            db_contents.serialize(&mut serializer).map_err(|e| io::Error::other(e))?;
        }
    ...
    pub async fn add_subscription(&self, subscription_info: SubscriptionInfo, floor: Floor) -> Result<(), io::Error> {
        ...
        self.access_db(&self.subscription_db_file_path, |subscriptions: &mut Subscriptions| {
    ...
    pub async fn get_subscriptions(&self, floor: Floor) -> Result<HashSet<SubscriptionInfo>, io::Error> {
        let subscriptions = self.access_db::<Subscriptions>(&self.subscription_db_file_path, |_| DbOperationResult::OnlyRead).await?;
    ...
    pub async fn get_banana_states(&self) -> Result<BananaState, io::Error> {
        let state = self.access_db::<BananaState>(&self.banana_state_db_file_path, |_| {
    ...
    pub async fn set_banana_state_for_floor(&self, floor: Floor, has_banana: bool) -> Result<(), io::Error> {
        
        self.access_db(&self.banana_state_db_file_path, |banana_state: &mut BananaState| {
...
mod tests {
    ...
    async fn test_database_add_and_retrieve_subscription() {
        ...
        let db_options = DatabaseOptions { db_file_prefix: Some(db_file_prefix.into()) };
    ...
    async fn test_banana_state_set_and_get_single_floor() {
        ...
        let states = db.get_banana_states()
            .await
            .expect("Failed to get banana states");

        // Verify the banana state was set correctly
        assert_eq!(states.len(), 1, "Should have exactly one floor with banana state");
        assert_eq!(states.get(&floor), Some(&true), "Floor 5 should have banana (true)");
        ...
        let updated_states = db.get_banana_states()
            .await
            .expect("Failed to get updated banana states");

        assert_eq!(updated_states.get(&floor), Some(&false), "Floor 5 should not have banana (false)");
    ...
    async fn test_banana_state_multiple_floors() {
        ...
        assert_eq!(states.len(), 3, "Should have exactly three floors with banana states");
        assert_eq!(states.get(&floor_1), Some(&true), "Floor 1 should have banana (true)");
        assert_eq!(states.get(&floor_2), Some(&false), "Floor 2 should not have banana (false)");
        assert_eq!(states.get(&floor_3), Some(&true), "Floor 3 should have banana (true)");
        ...
        assert_eq!(updated_states.get(&floor_1), Some(&true), "Floor 1 should still have banana");
        assert_eq!(updated_states.get(&floor_2), Some(&true), "Floor 2 should now have banana");
        assert_eq!(updated_states.get(&floor_3), Some(&true), "Floor 3 should still have banana");
    ...
    async fn test_banana_state_does_not_corrupt_subscriptions() {
        ...
        let retrieved_subs_floor_1 = db.get_subscriptions(floor_1)
            .await
            .expect("Failed to retrieve subscriptions for floor 1");
        let retrieved_subs_floor_2 = db.get_subscriptions(floor_2)
            .await
            .expect("Failed to retrieve subscriptions for floor 2");

        assert_eq!(retrieved_subs_floor_1.len(), 1, "Floor 1 should still have one subscription");
        assert_eq!(retrieved_subs_floor_2.len(), 1, "Floor 2 should still have one subscription");

        let sub_1 = retrieved_subs_floor_1.iter().next().expect("No subscription found");
        let sub_2 = retrieved_subs_floor_2.iter().next().expect("No subscription found");

        assert_eq!(sub_1.endpoint, subscription_info_1.endpoint, "Subscription 1 endpoint corrupted");
        assert_eq!(sub_1.keys.auth, subscription_info_1.keys.auth, "Subscription 1 auth corrupted");
        assert_eq!(sub_2.endpoint, subscription_info_2.endpoint, "Subscription 2 endpoint corrupted");
        assert_eq!(sub_2.keys.auth, subscription_info_2.keys.auth, "Subscription 2 auth corrupted");
    ...
    async fn test_subscription_changes_do_not_corrupt_banana_state() {
        ...
        assert_eq!(states.len(), 3, "Should still have three floors with banana states");
        assert_eq!(states.get(&floor_1), Some(&true), "Floor 1 banana state corrupted");
        assert_eq!(states.get(&floor_2), Some(&false), "Floor 2 banana state corrupted");
        assert_eq!(states.get(&floor_3), Some(&true), "Floor 3 banana state corrupted");
